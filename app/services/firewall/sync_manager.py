...
def sync_all(device_id):
    ...
    # 시스템 정보 동기화
    success, message = sync_system_info(device_id, is_batch=True, batch_id=batch_id)
    results.append({"type": "system_info", "success": success, "message": message})
    
    # 정책 동기화
    success, message = sync_firewall_policies(device_id, is_batch=True, batch_id=batch_id)
    results.append({"type": "policies", "success": success, "message": message})
    
    # 네트워크 객체 동기화
    success, message = sync_network_objects(device_id, is_batch=True, batch_id=batch_id)
    results.append({"type": "network_objects", "success": success, "message": message})
    
    # 네트워크 그룹 동기화
    success, message = sync_network_groups(device_id, is_batch=True, batch_id=batch_id)
    results.append({"type": "network_groups", "success": success, "message": message})
    
    # 서비스 객체 동기화
    success, message = sync_service_objects(device_id, is_batch=True, batch_id=batch_id)
    results.append({"type": "service_objects", "success": success, "message": message})
    
    # 서비스 그룹 동기화
    success, message = sync_service_groups(device_id, is_batch=True, batch_id=batch_id)
    results.append({"type": "service_groups", "success": success, "message": message})
    
    # 정책 사용 이력 동기화 (기본 30일)
    success, message = sync_usage_logs(device_id, days=30, is_batch=True, batch_id=batch_id)
